...
absl::Status MelFilterbank::Initialize(int fft_length, double sample_rate,
  ...
  if (num_mel_channels_ < 1) {
    return absl::InvalidArgumentError(
        "Number of mel filterbank channels must be positive.");
  }
  ...
  if (fft_length_ < 2) {
    return absl::InvalidArgumentError("FFT length must greater than 1.");
  }

  if (lower_frequency_limit < 0) {
    return absl::InvalidArgumentError(
        "Lower frequency limit must be nonnegative.");
  }

  if (upper_frequency_limit <= lower_frequency_limit) {
    return absl::InvalidArgumentError(absl::StrCat(
        "Upper frequency limit must be greater than lower frequency limit.",
        lower_frequency_limit, " vs ", upper_frequency_limit));
  }
  ...
  for (int i = 0; i < fft_length_; ++i) {
    if (i < start_index_ || i > end_index_) {
      ...
      if (channel < -2 || channel >= num_mel_channels_) {
        return absl::InternalError(
            absl::StrCat("Invalid mel channel: ", channel));
      }
  ...
  if (!bad_channels.empty()) {
    ABSL_LOG(WARNING) << "Missing " << bad_channels.size() << " bands "
...
absl::Status MelFilterbank::ToMelSpectrum(
    absl::Span<const double> squared_magnitude_fft,
    std::vector<double>* mel) const {
  if (!initialized_) {
    return absl::InternalError("Mel Filterbank not initialized.");
  }

  if (squared_magnitude_fft.size() <= end_index_) {
    return absl::InternalError("FFT too short to compute filterbank");
  }
...
absl::Status MelFilterbank::ToSquaredMagnitudeFft(
  ...
  if (mel.size() != num_mel_channels_) {
    return absl::InternalError(
        "Mel size does not match number of mel channels.");
  }
