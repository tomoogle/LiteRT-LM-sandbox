...
TEST(MelFilterbankTest, AgreesWithPythonGoldenValues) {
  ...
  ASSERT_OK(filterbank.Initialize(input.size(),
                                  /*sample_rate=*/22050, kChannelCount,
                                  /*lower_frequency_limit=*/20.0,
                                  /*upper_frequency_limit=*/4000.0));

  std::vector<double> output;
  ASSERT_OK(filterbank.ToMelSpectrum(input, &output));
...
TEST(MelFilterbankTest, IgnoresExistingContentOfOutputVector) {
  ...
  ASSERT_OK(filterbank.Initialize(kSampleCount,
                                  /*sample_rate=*/22050, kChannelCount,
                                  /*lower_frequency_limit=*/20.0,
                                  /*upper_frequency_limit=*/4000.0));
  ...
  ASSERT_OK(filterbank.ToSquaredMagnitudeFft(input, &output));
  EXPECT_THAT(output, Each(0.0));
}
...
TEST(MelFilterbankTest, UpperEdgeAtFftBintIsOk) {
  ...
  MelFilterbank filterbank;
  ASSERT_OK(filterbank.Initialize(kSampleCount, kSampleRateHz, kChannelCount,
...
TEST(MelFilterbankTest, InverseIsCloseToOriginal) {
  ...
  ASSERT_OK(filterbank.Initialize(kFftLength,
                                  /*sample_rate=*/22050, kChannelCount,
                                  /*lower_frequency_limit=*/20.0,
                                  /*upper_frequency_limit=*/4000.0));

  // Test with all mel bins equal to zero.
  ASSERT_OK(filterbank.ToSquaredMagnitudeFft(mel_filterbank,
                                             &estimated_squared_magnitude_fft));
  ASSERT_OK(filterbank.ToMelSpectrum(estimated_squared_magnitude_fft,
